...
const { logger, formatDuration, calculateETA } = require('./utils');
...
class VideoEncoder extends EventEmitter {
  constructor(config, sftpManager = null) {
    super();
    this.config = config;
  ...
  async encodeVideo(inputPath, outputPath, onProgress = null) {
    ...
    try {
      ...
      // Load user config from server if available
      if (this.sftpManager && !this.userConfig) {
        try {
          this.userConfig = await this.sftpManager.loadUserConfig();
      ...
      if (this.gpuAvailable === null) {
        ...
        this.gpuAvailable = await VideoEncoder.testGpuSupport();
      ...
      return new Promise((resolve, reject) => {
        ...
        if (this.gpuAvailable) {
          ...
          command
            .videoCodec('libx265')
            .addOption('-preset', cpuPreset)
            .addOption('-crf', crf)
            .addOption('-profile:v', profile)
            .addOption('-x265-params', 'log-level=error');
        }
      ...
      return new Promise((resolve) => {
        ...
        setTimeout(() => {
          if (this.currentProcess) {
            ...
            this.isEncoding = false;
            this.currentProcess = null;
            logger.warn('Encoding process force killed');
            resolve();
          }
        }, 10000);
      });
    ...
    if (state && state.outputPath) {
      try {
        if (await fs.pathExists(state.outputPath)) {
          await fs.remove(state.outputPath);
  ...
  static async testGpuSupport() {
    return new Promise((resolve) => {
      ffmpeg()
        .input('color=black:size=64x64:duration=1:rate=1')
        .inputFormat('lavfi')
        .videoCodec('hevc_nvenc')
        .addOption('-preset', 'p1')
        .addOption('-cq', 28)
        ...
        .on('end', () => {
          // Clean up test file
          fs.unlink('test_gpu.mp4').catch(() => {});
          logger.info('GPU encoding test successful');
          resolve(true);
        })
        .on('error', (err) => {
          logger.warn('GPU encoding test failed:', err.message);
          resolve(false);
        })
        .run();
    });
    ...
    return new Promise((resolve, reject) => {
      ffmpeg()
        .getAvailableEncoders((err, encoders) => {
          ...
          const nvidiaEncoders = Object.keys(encoders).filter(name => 
            name.includes('nvenc') || name.includes('cuda')
          );
  ...
  static estimateEncodingTime(videoInfo, preset = 'p7') {
    ...
    if (pixels > 3840 * 2160) { // 4K+
      baseTimePerMinute *= 3;
    } else if (pixels > 1920 * 1080) { // 1440p/2K
      baseTimePerMinute *= 2;
    } else if (pixels <= 1280 * 720) { // 720p or lower
      baseTimePerMinute *= 0.5;
    }
    
    // Adjust based on preset (p1 = fastest, p7 = balanced)
    const presetMultiplier = {
      'p1': 0.3, 'p2': 0.4, 'p3': 0.6, 'p4': 0.8,
      'p5': 1.0, 'p6': 1.2, 'p7': 1.5
    };
