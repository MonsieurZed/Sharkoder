...
class ProgressFileManager {
  constructor(config, sftpManager) {
    ...
    this.localCachePath = path.join(this.config.storage.local_temp, "progress_cache.json");
  ...
  async loadProgressFile() {
    try {
      ...
      const tempPath = path.join(this.config.storage.local_temp, "temp_progress.json");
  ...
  async uploadProgressFile(progressData) {
    try {
      ...
      const tempLocalPath = path.join(this.config.storage.local_temp, "upload_progress.json");
  ...
  async getStats() {
    try {
      ...
      if (progressData.jobs.length > 0) {
        const validJobs = progressData.jobs.filter((job) => job.original_size_mb > 0 && job.encoded_size_mb > 0);
        ...
        if (validJobs.length > 0) {
          ...
          stats.totalEncodingTimeHours =
            progressData.jobs.reduce((sum, job) => {
