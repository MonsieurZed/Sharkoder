...
class WebDAVExplorer {
  ...
  async connect() {
    try {
      ...
      let webdavUrl = this.config.remote.webdav.url || "http://ds10256.seedhost.eu:13888";
  ...
  async getVideoInfo(remotePath) {
    try {
      ...
      const fullPath = path.posix.join(this.config.remote.webdav.path || this.config.remote.sftp.path || "/", remotePath);
      const webdavUrl = this.config.remote.webdav.url;
      const webdavUser = this.config.remote.webdav.usernamename || this.config.remote.webdav.username;
      const webdavPass = this.config.remote.webdav.password;
      ...
      const scanDirectory = async (dirPath, parentPath = null, depth = 0) => {
        ...
        try {
          ...
          if (subDirectories.length > 0) {
            ...
            for (let i = 0; i < subDirectories.length; i += MAX_PARALLEL) {
              const batch = subDirectories.slice(i, i + MAX_PARALLEL);
              await Promise.all(batch.map((subDir) => scanDirectory(subDir, dirPath, depth + 1)));
            }
          }
        } catch (error) {
          logger.warn(`Could not scan directory ${dirPath}:`, error.message);
